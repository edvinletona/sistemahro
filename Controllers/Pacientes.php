...
class Pacientes extends Controllers
{
			...
			if (empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtEdad']) || empty($_POST['txtTelefono']) || empty($_POST['txtDireccion']) || empty($_POST['txtResponsable']) || empty($_POST['txtTelefonoResp'])) {
				...
				if ($idUsuario == 0) {
					...
					if ($_SESSION['permisosMod']['w']) {
						$request_user = $this->model->insertPaciente(
							$strIdentificacion,
							$strNombre,
							$strApellido,
							$intEdad,
	...
	/**
	 * Funcion para buscar un usuario por medio del nombre o apellido. 
	 */
	public function searchUserByMultipleFields($str_buscar)
	{
		if ($_SESSION['permisosMod']['r']) {
			$str_buscar = strClean($str_buscar);
			$arrData = $this->model->searchByUserNameLastnameDPI($str_buscar);
			if (empty($arrData)) {
				$arrResponse = array('status' => false, 'msg' => 'Datos no encontrados.');
			} else {
				$arrResponse = array('status' => true, 'data' => $arrData);
