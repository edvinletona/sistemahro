...
class Servicios extends Controllers
{
            ...
            if (empty($_POST['txtNombre']) || empty($_POST['listStatus'])) {
                ...
                if ($intIdServicio == 0) {
                    ...
                    if ($_SESSION['permisosMod']['w']) {
                        $request_servicio = $this->model->insertServicio(
                            $strServicio,
                            $intStatus
                        );
                    }
                    ...
                    if ($_SESSION['permisosMod']['u']) {
                        $request_servicio = $this->model->updateServicio(
                            $intIdServicio,
                            $intStatus
                        );
                    }
    ...
    /**
     * Obtiene un listado de todos los servicios
     */
    public function getServicesList(){
        $arrData = $this->model->listServices();
        if (empty($arrData)) {
            $arrResponse = array('status' => false, 'msg' => 'Datos no encontrados.');
        } else {
            $arrResponse = array('status' => true, 'data' => $arrData);
