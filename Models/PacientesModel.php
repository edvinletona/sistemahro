...
class PacientesModel extends Mysql
{
	...
	/**
	 * Buscar un usuario por coincidencia en nombre / apellido
	 */
	public function searchByUserNameLastnameDPI($serach_val)
	{
		$this->serach_value = $serach_val;
		if ($this->serach_value != "") {
			$sql = "SELECT idpersona,identificacion, nombres, apellidos FROM  persona WHERE nombres like '%$this->serach_value%' OR apellidos like '%$this->serach_value%' OR identificacion like '%$this->serach_value%' LIMIT 10";
			$request = $this->select_all($sql);
			return $request;
		} else {
			throw new Error("Invalid empty input string");
		}
	}
}
